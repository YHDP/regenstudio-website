/**
 * scope-pdf-renderer.js — jsPDF native renderer for the JPA (Joint Project
 * Agreement) AST. Forked from dpa-pdf-renderer.js with cover/signature labels
 * swapped (Dutch DPA wording → English JPA wording) and a few label-as-opt
 * pivot points so future engagement types (TruPASS, Wood Pass) can override.
 *
 * Walks the AST produced by RegenScopeMD.parse() and emits a multi-page A4
 * PDF with selectable text. Same architecture as dpa-pdf-renderer.js.
 *
 * Public API:
 *   RegenScopePDF.render(ast, opts) → Promise<Blob>
 *
 * opts shape:
 *   {
 *     cover: {
 ...
 *     coverHeroSrc: string,                // path to scope hero SVG (default '/Images/scope-cover-hero-c2.svg')
 *     filename: string,
 *   }
 *
...
(function (root) {
  ...
  async function loadImageAsPngFull(url, scale) {
    scale = scale || 4;
  ...
  // Backwards-compatible shim — most callers only need the data URL.
  async function loadImageAsPng(url, scale) {
    return (await loadImageAsPngFull(url, scale)).dataUrl;
  }
  ...
  function renderPageFooter(state) {
    ...
    doc.text('Page ' + state.pageNumber, PAGE.marginLeft, footY);
  ...
  function stampWatermark(state) {
    ...
    doc.text('DRAFT — NOT YET SIGNED', PAGE.width / 2, PAGE.height / 2, {
      align: 'center',
      angle: -30,
    });
    if (typeof doc.setGState === 'function' && typeof doc.GState === 'function') {
  ...
  function renderCover(state, cover, heroPng, logoPng, partyBLogo) {
    const doc = state.doc;

    // ----- Hero watercolor at TOP of page (full bleed left-to-right) -----
    // 595×250 SVG; place from y=0 to y=200 to leave room for content below.
    if (heroPng) {
      try {
    ...
    // ----- Brand-anchor line at y=222 -----
    // Single-party (DPA-style):  [Regen icon] REGEN STUDIO
    // Two-party (JPA bilateral): [Regen icon] REGEN STUDIO  ·  [Party-B logo]
    //
    // The Regen sub-anchor stays byte-identical to the DPA cover (same icon
    // size, same KoHo Bold "REGEN" + KoHo Light "STUDIO" wordmark, same
    // 8pt icon-to-wordmark gap). When partyBLogo is provided, the whole
    // anchor pair re-centres around a middot separator so both parties read
    // as visual equals.
    let y = 222;
    const fontSize = 17;
    const f = doc._regenFonts || {};
    ...
    const iconH = 30;
    const iconW = 30;
    const regenAnchorW = (logoPng ? iconW + 8 : 0) + wordW;

    // Party-B sub-anchor — height 44 (legibility-driven; many counterparty
    // logos integrate a wordmark inside their iconic mark and become unreadable
    // below ~40pt tall in print). Width is aspect-correct from natural dims.
    // Vertical CENTER aligns with the Regen icon center (y=215) — Regen icon
    // top is at y=200, so its center is at y=215; CTF center matches.
    let partyBW = 0;
    let partyBH = 0;
    let partyBTopY = 0;
    if (partyBLogo && partyBLogo.dataUrl) {
      partyBH = 44;
      partyBW = (partyBLogo.naturalW / partyBLogo.naturalH) * partyBH;
      partyBTopY = 215 - partyBH / 2;  // 193 — sits 7pt up into hero bottom-fade region (transparent)
    }

    // Separator dot between the two sub-anchors
    const sepDot = '·';
    const sepGap = 16;
    let sepW = 0;
    if (partyBLogo && partyBLogo.dataUrl) {
      if (hasKohoLight) doc.setFont('KoHo', 'bold'); else doc.setFont('helvetica', 'bold');
      sepW = doc.getTextWidth(sepDot);
    }

    const totalW = regenAnchorW + (partyBW ? sepGap + sepW + sepGap + partyBW : 0);
    const startX = (PAGE.width - totalW) / 2;

    // Draw Regen sub-anchor
    let cx = startX;
    if (logoPng) {
      try {
        doc.addImage(logoPng, 'PNG', cx, y - iconH + 8, iconW, iconH);
        cx += iconW + 8;
      } catch (err) {
        // Icon failed; advance only by wordmark gap
      }
    }
    if (hasKohoLight) doc.setFont('KoHo', 'bold'); else doc.setFont('helvetica', 'bold');
    doc.text('REGEN', cx, y);
    cx += regenW + spaceW;
    if (hasKohoLight) doc.setFont('KoHo-Light', 'normal'); else doc.setFont('helvetica', 'normal');
    doc.text('STUDIO', cx, y);
    cx += studioW;

    // Draw separator + Party-B sub-anchor (only when partyBLogo present)
    if (partyBW) {
      cx += sepGap;
      if (hasKohoLight) doc.setFont('KoHo', 'bold'); else doc.setFont('helvetica', 'bold');
      setColor(doc, COLOR.muted);
      doc.text(sepDot, cx, y);
      setColor(doc, COLOR.ink);
      cx += sepW + sepGap;
      try {
        doc.addImage(partyBLogo.dataUrl, 'PNG', cx, partyBTopY, partyBW, partyBH);
      } catch (err) {
        console.warn('Cover party-B logo render failed (non-fatal):', err);
      }
    }

    // ----- Subtle divider + small subtitle line below brand -----
    // Push down a bit more when the bilateral cover anchor extends below
    // the Regen icon (CTF / Party-B logo at 44 tall reaches y=237; default
    // subtitle at y=244 would visually crowd it).
    y += partyBLogo ? 34 : 22;
    ...
    setColor(doc, COLOR.ink);
    doc.text(cover.documentTitle || 'Project Scope Agreement', PAGE.marginLeft, y);

    y += 22;
    setFontByRole(doc, 'cover-subtitle');
    doc.setFontSize(11);
    setColor(doc, COLOR.muted);
    const partiesLine = 'between ' + (cover.partyALegalName || 'Regen Studio B.V.') + ' and ' + (cover.partyBLegalName || cover.legalName || '—');
    ...
    y += 14;
    const labelW = 145;
    const rows = cover.metadataRows || [
      ['Project', cover.label || '—'],
      ['Governing law', cover.applicableLaw || 'German law; Berlin courts'],
      ['Master Agreement', cover.masterAgreementRef || '—'],
      ['Effective date', cover.effectiveDate || '—'],
      ['Term', cover.termSummary || '—'],
    ];
    ...
    doc.setFontSize(9.5);
    setColor(doc, COLOR.emerald);
    doc.text('Bilaterally signed', PAGE.marginLeft + 12, boxY + 14);

    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8.5);
    setColor(doc, COLOR.ink);
    const sigLine1 = 'Party A: ' + (cover.partyASignerName || 'Yvo Hunink') +
      ' (' + (cover.partyASignerRole || 'Bestuurder') + ') for ' + (cover.partyALegalName || 'Regen Studio B.V.') +
      ' — pre-rendered authorisation signature';
    const sigLine2 = 'Party B: ' + (cover.partyBSignerName || cover.controllerSignerName || '—') +
      ' (' + (cover.partyBSignerRole || cover.controllerSignerRole || '—') + ') for ' + (cover.partyBLegalName || cover.legalName || '—');
    const sigLine3 = 'Bilateral signing time: ' + (cover.signedAtDisplay || cover.signedAt || '—');
    const sigLine4 = 'Simple Electronic Signatures under eIDAS Art 25(1) · Reg (EU) 910/2014. Full evidence components in the audit-log + signed-PDF SHA chain.';
    ...
    doc.setFontSize(8);
    setColor(doc, COLOR.muted);
    doc.text('Page 1', PAGE.marginLeft, footY);
  ...
  function renderSignatureBlock(state, sig) {
    ...
    doc.setFontSize(11);
    setColor(doc, COLOR.ink);
    doc.text('Signatures', PAGE.marginLeft, state.cursorY);
    state.cursorY += 14;
    ...
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(8);
    doc.text('Party A', PAGE.marginLeft, boxY + boxH + 12);
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7.5);
    ...
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(7);
    const attest = 'Visual confirmation — the legal anchors are the server-side stored SHA-256 hash, token hash, signed_at timestamp, and IP/UA hashes in the JPA signing-event registry. Bilaterally signed with Simple Electronic Signatures under eIDAS Art 25(1) · Reg (EU) 910/2014. Signature time (server-recorded): ' + (sig.signedAt || '—') + '.';
  ...
  async function render(ast, opts) {
    ...
    if (!root.jspdf || !root.jspdf.jsPDF) {
      throw new Error('jsPDF not loaded — required for RegenScopePDF.render()');
    }
    ...
    if (opts.cover) {
      let heroPng = null;
      let logoPng = null;
      let partyBLogo = null;  // { dataUrl, naturalW, naturalH } — bilateral cover only
      if (opts.coverHeroSrc !== false) {
        try {
          heroPng = await loadImageAsPng(opts.coverHeroSrc || '/Images/scope-cover-hero-c2.svg', 4);
        } catch (err) {
      ...
      if (opts.coverPartyBLogoSrc) {
        try {
          partyBLogo = await loadImageAsPngFull(opts.coverPartyBLogoSrc, 4);
        } catch (err) {
          console.warn('Cover party-B logo load failed (rendering single-anchor):', err.message);
        }
      }
      renderCover(state, opts.cover, heroPng, logoPng, partyBLogo);
    }
