/**
 * scope-md-render.js — Markdown parser for the JPA (Joint Project Agreement)
 * signing flow. Forked from dpa-md-render.js (the parser is doc-agnostic; the
 * only change is the registered global name `RegenScopeMD` so JPA + DPA can
 * coexist on the same page without colliding).
 *
 * Pipeline (post Phase-1 refactor):
 *
 *   markdown source ──► RegenScopeMD.parse(md, vars) ──► AST nodes ──► RegenScopeMD.astToHtml(ast) ──► HTML string
 ...
 *
 * Public API:
 *   RegenScopeMD.parse(md, vars)         → AST[]      (canonical primary path)
 *   RegenScopeMD.astToHtml(ast)          → string     (browser DOM injection)
 *   RegenScopeMD.render(md, vars)        → string     (backward-compat wrapper)
 *
 ...
 *   RegenScopeMD.escapeHtml, .stripFrontmatter, .substituteVars  (preserved utilities)
 */
